...
import { validateDocuments } from '../../lib/groqClient';
...
export default function Step5Documents({ policy }) {
  ...
  const handleFileUpload = (docTitle) => {
    setUploadedFiles((prev) => ({
      ...prev,
      [docTitle]: { name: `${docTitle.replace(/\s+/g, '_').toLowerCase()}.pdf`, status: 'UPLOADED' },
    }));
  };
  ...
  return (
    <div>
      ...
      <div className="mt-8">
        <div className="mb-4">
          <p className="text-xs text-gray-500 tracking-wider">ENHANCEMENT #3</p>
          <h3 className="text-xl font-bold text-gray-900">Document Compliance Checker</h3>
          <p className="text-xs text-gray-400 mt-1">Verify your documents match policy requirements BEFORE submission</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs text-gray-500 tracking-wider">DOCUMENT COMPLIANCE STATUS</p>
                <button
                  onClick={runComplianceCheck}
                  disabled={loadingCompliance}
                  className="text-xs bg-teal-700 text-white px-3 py-1.5 rounded-lg font-semibold hover:bg-teal-800 disabled:bg-gray-300"
                >
              ...
              <div className="space-y-3">
                {sampleDocuments.map((doc, i) => {
                  const isUploaded = uploadedFiles[doc.title] || doc.status === 'VERIFIED & ATTACHED';
                  const complianceStatus = compliance?.documentChecklist?.find((c) => c.document === doc.title);
                  return (
                    <div key={i} className={`rounded-lg border p-4 ${
                      !isUploaded ? 'border-amber-200 bg-amber-50' : 'border-green-200 bg-green-50'
                    }`}>
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3">
                          <div className="mt-0.5">
                            {isUploaded ? <CheckCircle size={18} className="text-green-600" /> : <AlertTriangle size={18} className="text-amber-600" />}
                          </div>
                          <div>
                            <p className="font-semibold text-sm text-gray-900">{doc.title}</p>
                            <p className={`text-xs font-semibold ${doc.status === 'VERIFIED & ATTACHED' ? 'text-green-600' : 'text-amber-600'}`}>
                              {doc.status === 'VERIFIED & ATTACHED' ? 'VERIFIED & ATTACHED' : isUploaded ? 'UPLOADED' : 'MISSING'}
                            </p>
                            {complianceStatus && (
                              <p className={`text-xs mt-1 ${
                                complianceStatus.compliance === 'PASS' ? 'text-green-700' :
                                complianceStatus.compliance === 'FAIL' ? 'text-red-700' : 'text-amber-700'
                              }`}>
                                {complianceStatus.compliance === 'PASS' ? 'All required fields present' :
                                 complianceStatus.compliance === 'FAIL' ? 'Missing required information' :
                        ...
                        {!isUploaded && (
                          <button
                            ...
                            className="bg-amber-600 text-white text-xs px-3 py-1.5 rounded-lg font-semibold flex items-center gap-1 hover:bg-amber-700"
                          >
                      ...
                      {complianceStatus?.suggestions && (
                        <p className="text-xs text-gray-600 mt-2 ml-7">{complianceStatus.suggestions}</p>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="space-y-4">
            {compliance && (
              <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                <p className="text-xs text-gray-500 tracking-wider mb-3">COMPLIANCE SCORE</p>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-gray-700">Overall</span>
                  <span className={`text-2xl font-bold ${
                    compliance.overallCompliance >= 80 ? 'text-green-600' :
                    compliance.overallCompliance >= 50 ? 'text-amber-600' : 'text-red-600'
                  }`}>{compliance.overallCompliance}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
                  <div
                    className={`h-3 rounded-full transition-all ${
                      compliance.overallCompliance >= 80 ? 'bg-green-500' :
                      compliance.overallCompliance >= 50 ? 'bg-amber-500' : 'bg-red-500'
                    }`}
                    style={{ width: `${compliance.overallCompliance}%` }}
                  />
                </div>
                <p className={`text-sm font-semibold ${
                  compliance.readyForSubmission ? 'text-green-700' : 'text-red-700'
                }`}>
                  {compliance.readyForSubmission ? 'Ready to submit' : 'Not ready yet'}
                </p>
              </div>
            )}
            ...
            {compliance?.riskFactors?.length > 0 && (
              <div className="bg-red-50 rounded-xl border border-red-200 p-5">
                <p className="text-xs font-bold text-red-900 mb-2">RISK FACTORS</p>
                <ul className="space-y-1">
                  {compliance.riskFactors.map((risk, i) => (
                    <li key={i} className="text-sm text-red-800 flex gap-2">
                      <XCircle size={14} className="mt-0.5 flex-shrink-0" /> {risk}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="bg-teal-50 rounded-xl border border-teal-200 p-5">
              ...
              <p className="text-sm text-teal-900">
                The compliance checker validates document completeness against your policy's requirements. In production, it would also verify document content (invoice dates, hospital headers, etc.) using OCR.
              </p>
            </div>
          </div>
        </div>
